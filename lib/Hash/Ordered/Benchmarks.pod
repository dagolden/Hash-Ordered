...
B<Note>: since the initial benchmarking, C<Hash::Ordered> gained just-in-time
indexing of the keys array to support faster tombstone deletion.  It also
now support C<tie>.  The revised benchmarks include the C<tie> mode
for consistency with other tied hash implementations.
